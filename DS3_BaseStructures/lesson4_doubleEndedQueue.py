'''
    ...
    双端队列：一系列元素的有序集合。其两端成为队首(front)和队尾(rear),元素在到达两端之前始终位于双端队列与队列不同的地方在于：双端的队列对元素的添加和删除限制不那么严格，元素可以从两端插入，也可以从两端删除
    总结来说：双端队列拥有栈和队列各自拥有的所有功能

    抽象数据类型(ADT)
        Deque()  创建一个空双端队列，五参数，返回值为Deque队象
        addFront(item)   在队首插入一个元素，参数为待插入元素，无返回值
        addRear(item)    在队尾插入一个元素。参数为待插入元素，无返回值
        removeFront()    在队首移出一个元素，无参数，返回该移出的元素，双端队列会被改变
        removerRear()    在队尾移出一个元素，无参数，返回该移出的元素，双端队列会被改变
